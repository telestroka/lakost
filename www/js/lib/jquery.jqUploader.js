 ...
 **/
 jQuery.fn.jqUploader = function(options) {
    return this.each(function(index) {
		...
		var startMessage = ($thisLabel.text() =='') ? 'Пожалуйста, выберите файл' : $thisLabel.text();
		...
		var opts = jQuery.extend({
				...
				src:    '/i/jqUploader/jqUploader.swf',
				...
				errorSizeMessage: 'Файл слишком большой!',
				validFileMessage: 'Нажмите Upload',
				progressMessage: 'Пожалуйста подождите... ',
				endMessage:    'Готово'
		}, options || {}
		);
		...
		TerminateJQUploader = function(containerId,filename,varname){
			...
			$this.text('').append('<span style="color:#000">Загрузка <strong>'+filename+'</strong> завершена!</span><input name="'+varname+'" type="hidden" id="'+varname+'" value="'+filename+'"/>');
